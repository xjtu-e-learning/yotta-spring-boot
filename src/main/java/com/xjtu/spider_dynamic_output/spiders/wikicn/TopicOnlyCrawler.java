...
import com.xjtu.spider_topic.spiders.wikicn.TopicCrawlerDAO;
...
public class TopicOnlyCrawler {
    ...
    public static void setDomainLanguage() {
        if (SpiderDynamicOutputService.getDomainFlag()) {domain_url0 = domain_url1;}
    ...
    public static void storeTopic(Domain domain) throws Exception {
         ...
         * 判断该课程领域术语是否已经爬取
         */
        Boolean existLayer = com.xjtu.spider_topic.spiders.wikicn.MysqlReadWriteDAO.judgeByClass1(Config.TOPIC_TABLE, Config.DOMAIN_TABLE, domainName);
        if (!existLayer) {
         ...
         * 判断该主题是否已经爬取
         */
        Boolean existTopic = com.xjtu.spider_topic.spiders.wikicn.MysqlReadWriteDAO.judgeByClass1(Config.TOPIC_TABLE, Config.DOMAIN_TABLE, domainName);
        if (!existTopic) {
