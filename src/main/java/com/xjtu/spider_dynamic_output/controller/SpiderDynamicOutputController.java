...
public class SpiderDynamicOutputController {
    ...
    @ApiOperation(value = "输入课程名、主题名，返回主题下的分面树形结构", notes = "输入课程名、主题名，返回主题下的分面树形结构")
    @GetMapping("/getFacetByDomainAndTopicName")
    public ResponseEntity getFacetByDomainAndTopicName(@RequestParam(name="domainName") String domainName,
                                                            @RequestParam(name="topicName") String topicName
    ) throws Exception{
        Result result=spiderdOutputService.getFacetByDomainAndTopicName(domainName,topicName);
        if (!result.getCode().equals(ResultEnum.SUCCESS.getCode())) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(result);
        }
        return ResponseEntity.status(HttpStatus.OK).body(result);
