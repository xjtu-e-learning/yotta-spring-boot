...
//    }
     ...
     * @return
     */
    public static List<Facet> getTopicFacet(Long topicId) {
        List<Facet> facetList = new ArrayList<Facet>();
        mysqlUtils mysql = new mysqlUtils();
        String sql = "select * from " + Config.FACET_TABLE + " where topic_id=?";
        List<Object> params = new ArrayList<Object>();
        params.add(topicId);
        try {
            List<Map<String, Object>> results = mysql.returnMultipleResult(sql, params);
            for (int i = 0; i < results.size(); i++) {
                Map<String, Object> result = results.get(i);
                Long facetId = Long.parseLong(result.get("facet_id").toString());
                Long topicID=Long.parseLong(result.get("topic_id").toString());

                String facetName = result.get("facet_name").toString();

                Facet facet = new Facet();
                facet.setFacetName(facetName);
                facet.setTopicId(topicID);
     ...
     * @return
     */
    public static void storeAssemble(String assembleContent, String assembleText, Long domainId, Long facetId, Long sourceId) {
